...
async function runApp() {
    ...
    if (cmdLine.options.config) {
        config_1.setupConfig(require(cmdLine.options.config));
    }
    else {
    ...
    return {
        ...
        action: async function (options) {
            ...
            for (let migration of migrations) {
                logger_1.default.log(migration.migsiName, migration.inDevelopment ? 'dev ' : 'prod', migration.runDate || 'to-be-run');
            }
        }
    };
}
function output() {
    return {
        ...
        action: async ({ name, since: rawSince, until: rawUntil, failed, raw }) => {
            const since = parseDate(rawSince), until = parseDate(rawUntil, 1);
            const migrations = (await core.filterMigrations({ name, since, until, failed }))
                .filter(m => m.hasBeenRun || m.failedToRun);
            for (let migration of migrations) {
                const linearRunOutput = output_tracker_1.outputProcessor.makeLinear(migration, 'run');
                ...
                const linearRollbackOutput = output_tracker_1.outputProcessor.makeLinear(migration, 'rollback');
    ...
    function parseDate(dateStr, dayOffset = 0) {
        if (!dateStr)
            return undefined;
        const isFullISOString = /^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d.\d{3}.+$/, isDuration = /^(\d+)\s*(\w+)$/;
        if (isFullISOString.test(dateStr))
            return new Date(dateStr);
        if (isDuration.test(dateStr)) {
            const match = dateStr.match(isDuration) || [];
            const [, amount, unit] = match;
            const duration = moment.duration(parseInt(amount), unit);
...
function createTemplate() {
    return {
        ...
        action: async function ({ name: rawName }) {
            ...
            logger_1.default.log('Template created as ', path.relative(process.cwd(), filename));
...
function create() {
    return {
        ...
        async action({ friendlyName, template = 'default' }) {
            ...
            logger_1.default.log('Migration script created: ' + filename);
        }
    };
}
async function createWizard() {
    ...
    logger_1.default.log('The script can be found to be edited at ' + path.relative(process.cwd(), filename));
}
function getTemplates() {
    const customTemplateDir = config_1.getDir("templateDir");
...
function* getTemplatesFrom(dir, prefix = '') {
    const isJS = /\.js$/, isTemplateJS = /\.template\.js$/;
