...
function sortMigrations(unsorted: Migration[]) {
  const dependencyMap = _.fromPairs(
    unsorted.map((migration) => [migration.migsiName, migration.dependencies.length ? migration.dependencies : ['*']])
  )
...
function fixParallelDependencyOrder(migrations: Migration[]) {
  ...
  for (let i = 0; i < migs.length - 1; ++i) {
    ...
    if (a.toBeRun && !b.eligibleToRun) {
      ...
      // b could be run, but doesn't need to be
      if (!b.dependencies.includes(a.migsiName)) {
        // reorder them to for ideal order
        migs.splice(i, 2, b, a)
        i = Math.max(i - 2, -1)
      }
    } else if (a.inDevelopment && !b.inDevelopment && a.toBeRun === b.toBeRun) {
      // production scripts before development
      if (!b.dependencies.includes(a.migsiName)) {
        // reorder them to for ideal order
        migs.splice(i, 2, b, a)
        i = Math.max(i - 2, -1)
...
function checkMigrationOrderValidity(migrations: Migration[], dependenciesHaveBeenUpdated = false) {
  ...
  for (let migration of migrations) {
    if (
      migration.toBeRun ||
      (anyToRun && config.allowRerunningAllMigrations) ||
      (anyToRun && migration.eligibleToRun && migration.archived)
    ) {
      anyToRun = true
      ...
      if (!anyToRun) {
        ...
        if (dependenciesHaveBeenUpdated) {
          logger.error(`${
            migration.migsiName
          } can not be run, as it should've been run before the migrations ${toBeRun.join(
            ', '
          )}, which ha(s|ve) not been run.
        
        To solve this problem, you can either:
        - add an explicit dependency to run ${migration.migsiName} before the others; usually by having ${
            migration.migsiName
          } depend on a migration script that
        ...
        logger.warn(
          `Some of the dependencies of ${migration.migsiName} are to be run despite it having been run already. This is permitted as the script is still under development. If you need it run as well, please increment its version number.`
        )
      }
    }
  }
...
export const findMigrations = async function findMigrations(dependenciesUpdated = false): Promise<Migration[]> {
  ...
  const migrations = await Promise.all(
    files.map(async function (file) {
      ...
      migration.eligibleToRun = Boolean(
        !past || !past.hasBeenRun || !!past.inDevelopment || (config.allowRerunningAllMigrations && migration.toBeRun)
      )
      return migration
    })
  )
